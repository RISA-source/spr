...
class UnsupervisedLearner:
    ...
    def __init__(
        ...
        candidate_match_threshold: float = 0.28,  # v2.3: lowered for better merging
    ...
    def observe(self, sdr: SDR, label: Optional[str] = None) -> Optional[MatchResult]:
        ...
        # After consolidation, check if any candidates now meet promotion threshold
        for cand in list(self._candidates):
            if cand.count >= self.promotion_threshold:
                lbl = label or cand.label or f"p{self._n_promoted}"
                self.memory.learn(lbl, cand.sdr)
                self._candidates.remove(cand)
                self._n_promoted += 1
                if lbl not in self._confidence:
                    self._confidence[lbl] = PatternConfidence(
                        lbl, match_count=cand.count, total_score=float(cand.count) * 0.5
                    )
                return MatchResult(
                    label=lbl, score=1.0,
                    raw_overlap=cand.sdr.n_active(),
                    pattern=cand.sdr, input_sdr=sdr,
                )
    ...
    def _consolidate_candidates(self):
        """
        Multi-pass merge of the candidate pool.

        After adding a new candidate, pairs that were individually below the
        merge threshold may become mergeable once bundled with others.
        
        Example:
          cat1 vs cat2 = 0.07  (miss)
          cat1 vs cat3 = 0.37  (merge → cat13 bundle)
          cat13 vs cat2 = 0.40 (now mergeable — triggers)
        
        Runs until no more merges are possible in a full pass.
        """
        changed = True
        while changed:
            changed = False
            for i in range(len(self._candidates)):
                ...
                for j in range(i + 1, len(self._candidates)):
                    if j >= len(self._candidates):
                        break
                    ci = self._candidates[i]
                    cj = self._candidates[j]
                    if ci.sdr.overlap_score(cj.sdr) >= self.candidate_match_threshold:
                        # Merge cj into ci
                        ci.sdr = ci.sdr.bundle(cj.sdr)
                        ci.count += cj.count
                        ci.strength = min(1.0, ci.strength + cj.strength * 0.5)
                        self._candidates.pop(j)
                        changed = True
